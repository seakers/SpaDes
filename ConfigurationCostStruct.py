...
from StructAnalysis import structuralAnalysisMain
import miniball

def centerMassCost(locations, masses, structLocs, structDims, structOrients, structMasses):
    # Function to calculate the distance of the center of mass from the circumcenter of the structure

    # center of mass
    allLocs = locations + structLocs
    allMasses = masses + structMasses

    allLocs = np.array(allLocs)
    allMasses = np.transpose(np.array([allMasses,allMasses,allMasses]))

    centerMass = np.sum(allLocs*allMasses,0)/np.sum(allMasses,0)

    # circumcenter
    structPoints = np.zeros((len(structDims)*8, 3))
    for i in range(len(structDims)):
        ...
        nodes = nodes + locs
        structPoints[i*8:(i+1)*8] = nodes

    mb = miniball.Miniball(structPoints)
    circumcenter = mb.center()

    circumcenter = np.array(circumcenter)

    diff = np.sqrt(np.sum(np.square(centerMass - circumcenter)))
...
def overlapCostArbitrary(dimensions,locations,orientations, structDims, structLocs, structOrients):
    """
    Calls the separating axis test on each pair of components to see if they overlap
    If any pair overlaps, the loop is broken and true is returned to save time.
    Also check if any panel overlaps with any component, however panels overlapping each other is ok
    and in fact necessary to be a feasable structure
    """
    numComps = len(dimensions)
    numPanels = len(structDims)
    compPairList = itertools.combinations(range(numComps),2)
    for pair in compPairList:
        (ind1,ind2) = pair
        overlapBool = separatingAxisTest(dimensions[ind1], locations[ind1], orientations[ind1], dimensions[ind2], locations[ind2], orientations[ind2])
        if overlapBool:
            return True
    
    structCompPairList = itertools.product(range(numComps),range(numPanels))
    for structPair in structCompPairList:
        compInd,structInd = structPair
        overlapBool = separatingAxisTest(dimensions[compInd], locations[compInd], orientations[compInd], structDims[structInd], structLocs[structInd], structOrients[structInd])
        if overlapBool:
            return True
...
def separatingAxisTest(dim1,loc1,dcm1,dim2,loc2,dcm2):
    """
    Function to check if two arbitrary cuboids overlap using the separating axis theorem

    Parameters:
    - dim1: (length, width, height) of cuboid 1
    - loc1: (x, y, z) position of cuboid 1 center
    - dcm1: 3x3 direction cosine matrix for cuboid 1 orientation
    - dim2: (length, width, height) of cuboid 2
    - loc2: (x, y, z) position of cuboid 2 center
    - dcm2: 3x3 direction cosine matrix for cuboid 2 orientation

    Returns:
    - True if the cuboids overlap, False otherwise.
    """
    # Generate local axes of each cuboid
    axes1 = dcm1.T  # Local axes of cuboid 1
    axes2 = dcm2.T  # Local axes of cuboid 2

    # Compute edge half-lengths along each local axis
    half_sizes1 = np.array(dim1) / 2.0
    half_sizes2 = np.array(dim2) / 2.0

    # Compute the translation vector from cuboid 1 to cuboid 2
    t = np.array(loc2) - np.array(loc1)

    # Express translation vector in the frame of cuboid 1
    t_local1 = np.dot(axes1, t)

    # Compute the 3x3 matrix of dot products between local axes
    R = np.dot(axes1, axes2.T)

    # Compute absolute values of R with a small epsilon for numerical stability
    R_abs = np.abs(R) + 1e-6

    epsilon1 = 1e-6 # small margin to account for floating point errors and give more flexibility
    epsilon2 = 1e-6

    # Define the 15 possible separating axes
    for i in range(3):  # Cuboid 1's local axes
        ra = half_sizes1[i]
        rb = np.dot(half_sizes2, R_abs[i,:])
        if abs(t_local1[i]) > ra + rb - epsilon1:
            # print("PING! 1")
            return False

    for j in range(3):  # Cuboid 2's local axes
        ra = np.dot(half_sizes1, R_abs[:, j])
        rb = half_sizes2[j]
        if abs(np.dot(t, axes2[j])) > ra + rb - epsilon1:
            # print("PING! 2")
            return False

    for i in range(3):  # Cross products of local axes
        for j in range(3):
            ...
            if abs(t_local1[(i + 2) % 3] * R[(i + 1) % 3, j] - t_local1[(i + 1) % 3] * R[(i + 2) % 3, j]) + epsilon2 > ra + rb - epsilon1:
                # print("PING! 3")
                return False

    return True  # No separating axis found, cuboids overlap

def overlapCostSingleSAT(structPanels, components, newComponent):
    
    newCompDim = newComponent.dimensions
    newCompLoc = newComponent.location
    newCompOrient = newComponent.orientation

    for i in range(len(components)):
        compDim = components[i].dimensions
        compLoc = components[i].location
        compOrient = components[i].orientation
        overlapBool = separatingAxisTest(compDim, compLoc, compOrient, newCompDim, newCompLoc, newCompOrient)
        if overlapBool:
            return True

    for j in range(len(structPanels)):
        structDim = structPanels[j].dimensions
        structLoc = structPanels[j].location
        structOrient = structPanels[j].orientation
        overlapBool = separatingAxisTest(structDim, structLoc, structOrient, newCompDim, newCompLoc, newCompOrient)
        if overlapBool:
            return True
...
def pointingObj(dimensions,locations,types,orientations):
    ...
    for ind,comp in enumerate(types):
        if comp in pointingComps:
            ...
            newPointOrients.append(orientations[ind])
        
    return newPointLocs,newPointDims,newPointOrients

def structuralFeasability(structDims, structLocs, structOrients):
    """
    Checks if all structural panels are interconnected
    """
    numPanels = len(structDims)
    if numPanels <= 1:
        # print(True)
        # print("Number of Panels: ", numPanels)
        # print("Locations: ", structLocs)
        # print("Dimensions: ", structDims)
        return True

    graph = {panel: [] for panel in range(numPanels)}

    for i in range(numPanels):
        for j in range(i+1, numPanels):
            if separatingAxisTest(structDims[i],structLocs[i],structOrients[i],structDims[j],structLocs[j],structOrients[j]):
                graph[i].append(j)
                graph[j].append(i)
    ...
    connected = (len(visited) == numPanels)

    # print(connected)
    # if connected:
    #     print("Number of Panels: ", numPanels)
    #     print("Connected Panels: ", visited)
    #     print("Graph: ", graph)
    #     print("Locations: ", structLocs)
    #     print("Dimensions: ", structDims)
...
def structuralLoad(structDims, structLocs, structOrients, locations, masses):
    structLoadBool = structuralAnalysisMain(structDims, structLocs, structOrients, locations, masses)
    return structLoadBool


            
def constraintCost(dimensions, locations, types, orientations, masses, structDims, structLocs, structOrients):
    """
    ...
    newPointLocs,newPointDims,newPointOrients = pointingObj(dimensions,locations,types,orientations)

    structFeasBool = structuralFeasability(structDims, structLocs, structOrients)
    if structFeasBool:
        overlapBool = overlapCostArbitrary(dimensions + newPointDims, locations + newPointLocs, orientations + newPointOrients, structDims, structLocs, structOrients)
        if not overlapBool:
            structLoadBool = structuralLoad(structDims, structLocs, structOrients, locations, masses)
            if not structLoadBool:
                return False # no violation
        # return False #temporary to get some results
...
def getCostComps(components, structPanels, maxCosts):
    ...
    for panel in structPanels:
        ...
        structDims.append(panel.dimensions)
        structLocs.append(panel.location)
        structOrients.append(panel.orientation)

    # Get the cost from each cost source
    constraintCostBool = constraintCost(dimensions, locations, types, orientations, masses, structDims, structLocs, structOrients)
    cmCostCalVal = centerMassCost(locations, masses, structLocs, structDims, structOrients, structMasses)
    ...
    maxCM,maxOffAx,maxOnAx,maxWire,maxThermal = maxCosts

    costList = [cmCostCalVal/maxCM, offAxisInertia/maxOffAx, onAxisInertia/maxOnAx, wireCostVal/maxWire, thermalCostVal/maxThermal]
    return costList, constraintCostBool


def getCostCompsNonnormal(components, structPanels):
    # pull parameters from the components
    locations = []
    dimensions = []
    orientations = []
    ...
    costList = [cmCostCalVal, offAxisInertia, onAxisInertia, wireCostVal, thermalCostVal]
    return costList, constraintCostBool
