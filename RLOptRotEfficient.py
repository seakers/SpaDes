...
class Actor(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.rotation_output_layer = layers.Dense(units=6, activation='softmax') # 6 actions for each possible right angle orientation
    ...
    def sample_configuration(self, observations, rot):
        ...
        max_length = self.state_dim
        padded_observations = [obs + [0] * (max_length - len(obs)) for obs in observations]
        observations = tf.convert_to_tensor(padded_observations, dtype=tf.float32)
        obs_as_vector = tf.reshape(observations, [-1])
        zero_padding = tf.zeros([batch_size * self.state_dim] - tf.shape(obs_as_vector), dtype=observations.dtype)
        obs_padded = tf.concat([obs_as_vector, zero_padding], 0)
        input_observations = tf.reshape(obs_padded, [batch_size, self.state_dim])
