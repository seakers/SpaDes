...
def optimization(components,optMethod):
    ...
    #     allLocs,numSteps = gradientOptimization(components)
    #     dimensions = []
    #     for comp in components:
    #             # locations.append(comp.location)
    #         dimensions.append(comp.dimensions)
    #     allDims = [dimensions]*numSteps

    if optMethod == "GA":
        allLocs,allDims,numSteps,allMaxHV,allHV = GAOptimization(components)

    # elif optMethod == "GANR":
    #     allLocs,allDims,numSteps,allHV = GAOptimizationNR(components)

    elif optMethod == "RL":
        allLocs, allDims, avgCost, numSteps, allMaxHV, allHV = RLWrapper.run(components)

    for i in range(len(components)):
        components[i].location = allLocs[-1][i]
        components[i].dimensions = allDims[-1][i]

    # return allLocs,allDims,numSteps,allHV,allRots
