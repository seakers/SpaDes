...
def run_epoch(actor, critic, components,NFE):
    ...
    for idx, des in enumerate(designs):
        for i in range(len(components)):
            ...
            # Here every 4th coord gets mapped to a rotation. Simplest way I could think to do things. Other methods are difficult or slow.
            # Come back to this at some point and try to improve so we don't have to collapse the design space like this.
            dims = components[i].dimensions
            # if >= -1 and < -0.67 then unchanged
            if des[4*i+3] >= -0.67 and des[4*i+3] < -0.33:
                dims = [dims[0],dims[2],dims[1]]
            elif des[4*i+3] >= -0.33 and des[4*i+3] < 0:
                dims = [dims[1],dims[0],dims[2]]
            elif des[4*i+3] >= 0 and des[4*i+3] < 0.33:
                dims = [dims[1],dims[2],dims[0]]
            elif des[4*i+3] >= 0.33 and des[4*i+3] < 0.67:
                dims = [dims[2],dims[0],dims[1]]
            elif des[4*i+3] >= 0.67 and des[4*i+3] <= 1:
