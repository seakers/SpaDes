...
class Actor(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.num_actions_tran = 100
        self.num_actions_rot = 6
        ...
        self.output_layer_tran = layers.Dense(units=self.num_actions_tran, activation='softmax')
        self.output_layer_rot = layers.Dense(units=self.num_actions_rot, activation='softmax')



    def call(self, inputs, head, training=False):
        ...
        x = self.hidden_3(x)
        if head == "tran":
            x = self.output_layer_tran(x)
        elif head == "rot":
            x = self.output_layer_rot(x)

        return x


    def sample_configuration(self, observations, head):
        input_observations = []
        ...
        output = self(input_observations, head)  # shape: (num components, state variables)
    ...
    def ppo_update(self, observation_tensor, action_tensor, logprob_tensor, advantage_tensor):
        ...
        with tf.GradientTape() as tape:
            ...
            for obs in observation_tensor:
                ...
                prob = tf.squeeze(self.call(obs, head))
                pred_probs.append(prob)
                obsNum+=1
            # pred_probs = self.call(observation_tensor, "tran")  # (135, 100)
...
class Critic(tf.keras.Model):
    def __init__(self, **kwargs):
        ...
        self.hidden_2 = layers.Dense(units=64, activation='relu')
        self.hidden_3 = layers.Dense(units=64, activation='relu')
        self.output_layer = layers.Dense(units=5, activation='linear') # One output for each cost
...
class RLWrapper(): # Eventually come through and make code more flexible eg. based on the number of components as a variable (60 and 15 are hardcoded everywhere)
...
def run_epoch(actor, critic, components,NFE):
    ...
    for x in range(num_actions):
        if x % 4 == 3:
            ...
            head = "tran"
        log_probs, sel_actions, all_action_probs = actor.sample_configuration(observation, head)
    ...
    for idx, des in enumerate(designs):
        ...
        configs.append([allLocs,allDims])
